...
const solution = (picks, minerals) => {
  let answer = 0;
  const picksArr = picks; // 곡괭이 개수
  const sortArr = []; // 5개씩 자른 광물 [dia, iron, stone]의 개수가 담기는 2차원 배열
  const fatigue = [
    [1, 1, 1],
    [5, 1, 1],
    [25, 5, 1],
  ...
  const maxCount = picksArr.reduce((acc, cur) => acc + cur, 0);

  // 하나의 곡괭이로 5번 작업을 할 수 있기 때문에, 광물을 5개씩 나눈 길이 올림처리(총 작업할 수 있는 횟수)
  const mineralsCutLength = Math.ceil(minerals.length / 5);

  // 광물을 5개씩 자르고, 다이아몬드, 철, 돌의 개수를 카운트한다.
  ...
  sortArr.sort((a, b) => {
    ...
    if (a[0] === b[0]) {
      // 다음으로 철 개수를 비교하고
      // 철 개수마저 같다면 돌 개수를 비교한다.
      if (a[1] === b[1]) {
        return b[2] - a[2];
        // 철 개수가 다르다면 철 개수를 비교한다.
        // 철 개수가 많은 순으로 정렬한다.
      } else {
        return b[1] - a[1];
      }
    } else {
      // 다이아몬드 개수가 많은 순으로 정렬한다.
      return b[0] - a[0];
    }
  });
  ...
  sortArr.forEach((item) => {
    ...
    let idx = 0; // 사용할 곡괭이의 idx

    // 피로도를 계산하기 위해 곡괭이가 있을 경우 다이아 > 철 > 돌 순의로 idx를 설정한다.
    if (picksArr[0] !== 0) idx = 0;
    else if (picksArr[1] !== 0) idx = 1;
    else if (picksArr[2] !== 0) idx = 2;
